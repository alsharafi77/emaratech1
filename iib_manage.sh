...
start_iib()
{
	...
	if [ ${NODE_EXISTS} -ne 0 ]; then
	  ...
	  cp /opt/ibm/iib-10.0.0.11/server/ODBC/unixodbc/odbcinst.ini /var/mqsi/odbc/
	  export ODBCSYSINI=/var/mqsi/odbc/
	  mqsisetdbparms $NODENAME -n ESBDB  -u esb_admin_dev -p 'E_Dev#_2018'
          echo "----------------------------------------" 
          echo "----------------------------------------"
	  echo "Configuring embedded cache..."
	  mqsichangebroker $NODENAME -b default
	  mqsichangebroker $NODENAME -b default -r 3840-3859
	  echo "----------------------------------------"
          ...
          mqsistart $NODENAME
          echo "----------------------------------------" 
	  echo "continuing configuration of global cache.."
 	  mqsichangeproperties $NODENAME -b cachemanager -o CacheManager -n policy,portRange,listenerHost -v default,generate,slwesbibd01
          echo "----------------------------------------"
	  ...
	  echo "Verifying connection to database..."
	  mqsicvp $NODENAME -n ESBDB
          echo "----------------------------------------"
          echo "----------------------------------------"
          shopt -s nullglob
